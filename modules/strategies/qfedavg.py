...
from modules.strategies.fedavg import FedAvg
...
class QFedAvg(FedAvg):
    ...
    def __init__(
        ...
        *,
        ...
        on_fit_config_fn: Optional[Callable[[ClientProxy, ClientManager, int], Dict[str, Scalar]]] = None,
        on_evaluate_config_fn: Optional[Callable[[ClientProxy, int], Dict[str, Scalar]]] = None,
    ...
    def configure_fit(
        ...
        return self.assign_client_configs(config_fn=self.on_fit_config_fn, clients=clients, server_round=server_round,
                                          client_manager=client_manager, parameters=parameters, instructions="fit")
    ...
    def configure_evaluate(
        ...
        # Return client/config pairs
        return self.assign_client_configs(config_fn=self.on_fit_config_fn, clients=clients, server_round=server_round,
                                          client_manager=client_manager, parameters=parameters, instructions="evaluate")
    ...
    def assign_client_configs(self, config_fn: Callable[[ClientProxy, ClientManager, int], Dict[str, Scalar]],
                              clients: List[ClientProxy], client_manager: ClientManager, parameters: Parameters,
                              server_round: int, instructions: str) -> List[Tuple[ClientProxy, Union[FitIns, EvaluateIns]]]:
        """
        ...
        for client in clients:
            ...
            if self.on_fit_config_fn is not None:
                ...
                config = config_fn(client, client_manager, server_round)
            if instructions == "fit":
                ins = FitIns(parameters, config)
            else:
                ins = EvaluateIns(parameters, config)

            client_list.append((client, ins))
        return client_list
