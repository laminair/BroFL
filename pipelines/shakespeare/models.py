...
class ShakespeareLSTM(pl.LightningModule):
    ...
    def __init__(self, privacy_engine=None, *args, **kwargs):
        ...
        #     self.lstm = DPLSTM(input_size=self.config["emb_dim"], hidden_size=self.config["hidden_dim"],
        #                        num_layers=self.config["n_layers"], batch_first=True)
        # else:
        self.lstm = nn.LSTM(input_size=self.config["emb_dim"], hidden_size=self.config["hidden_dim"],
                            num_layers=self.config["n_layers"], batch_first=True)

        self.fc = nn.Linear(self.config["hidden_dim"], self.config["vocab_size"])
        self.optim = torch.optim.SGD(lr=self.config["lr"], params=self.parameters())
        ...
        self.acc = torchmetrics.Accuracy(task="multiclass", num_classes=self.config["vocab_size"])
        self.f1 = torchmetrics.F1Score(task="multiclass", num_classes=self.config["vocab_size"])
    ...
    def validation_epoch_end(self, outputs):
        super(ShakespeareLSTM, self).validation_epoch_end(outputs)
    ...
    def backward(self, loss, optimizer, optimizer_idx, *args, **kwargs) -> None:
        start_time = time.time()
        super(ShakespeareLSTM, self).backward(loss, optimizer, optimizer_idx, *args, **kwargs)
        self.log("timing/train/backward_s", time.time() - start_time)

    def on_train_epoch_start(self) -> None:
        super(ShakespeareLSTM, self).on_train_epoch_start()
        self.batch_load_time_start = time.time()

    def on_train_batch_start(self, batch, batch_idx):
        super(ShakespeareLSTM, self).on_train_batch_start(batch, batch_idx)
        self.log("timing/train/batch_load_time_s", time.time() - self.batch_load_time_start)

    def on_train_batch_end(self, outputs, batch, batch_idx):
        super(ShakespeareLSTM, self).on_train_batch_end(outputs, batch, batch_idx)
