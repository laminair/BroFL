...
import torch
from torchaudio.transforms import (MFCC as torch_MFCC, MelSpectrogram as torch_MelSpectrogram,
                                   Spectrogram as torch_Spectrogram)
...
class Spectrogram(object):
    ...
    def __init__(self, net_frequency=50, measurement_frequency=6400):
        ...
        self.hop_len = int((self.n_fft / 2) + 1)
        self.torch_spec = torch_Spectrogram(n_fft=self.n_fft, hop_length=self.hop_len)
    ...
    def __call__(self, sample):
        ...
        # spec = self.torch_spec(curntre_in)
        try:
            spec = self.torch_spec(current_in)
        except RuntimeError:
            # Disabled since librosa may cause issues on jetson nano. May be enabled for all other platforms.
            spec, _ = librosa.core.spectrum._spectrogram(current_in.numpy(), n_fft=self.n_fft, hop_length=self.hop_len,
...
class MelSpectrogram(object):
    ...
    def __init__(self, net_frequency=50, measurement_frequency=6400):
        ...
        self.torch_mel_spec = torch_MelSpectrogram(sample_rate=measurement_frequency, n_fft=self.n_fft,
...
class MFCC(object):
    ...
    def __init__(self, net_frequency=50, measurement_frequency=6400):
        ...
        self.n_fft = int(measurement_frequency / net_frequency) * 2 - 1
        self.hop_length = int((self.n_fft / 2) + 1)
        self.torch_mfcc = torch_MFCC(sample_rate=measurement_frequency, n_mfcc=64,
